...
const Step4 = () => {
  ...
  const sauce = useSelector(state => state.pizzaReducer.sauce)
  const [input, changeInput] = useState(sauce)

  const onSubmit = () => {
    dispatch(chooseSauce(input))
    history.push("./user") // NOTE: navigate to User.js component first then after filling user details go to result route from there
  }
